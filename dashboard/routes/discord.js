...
router.get("/login", async function (req, res) {
    if (!req.user || !req.user.id || !req.user.guilds){
        return res.redirect(`https://discordapp.com/api/oauth2/authorize?client_id=${req.client.user.id}&scope=identify%20guilds&response_type=code&redirect_uri=https://dash.invite-count.xyz/api/callback&state=${req.query.state || "no"}`);
    }
...
router.get("/callback", async (req, res) => {
   ...
   // if (!req.query.code) res.redirect(req.client.config.failureURL);
    ...
    params.set("redirect_uri", `https://dash.invite-count.xyz/api/callback`);
